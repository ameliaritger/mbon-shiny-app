...
```{r}
#Find number of times species makes an appearance
reef_present <- reef_tidy %>%
  ...
  mutate(to_match = ifelse(grouped_genus==FOCAL, filename, "FALSE")) %>% #create a column that we can subset all rows in a plot based on the presence of focal phylum in the plot at least once
...
#Find number of times species co-occurs with neighbor species
reef_neighbor <- reef_tidy %>%
  ...
  mutate(to_match = ifelse(grouped_genus==FOCAL, filename, "FALSE")) %>% #create a column that we can subset all rows in a plot based on the presence of focal phylum in the plot at least once
  filter(filename %in% to_match) %>% #if focal genus is present, keep all observations of that plot ("filename")
  filter(grouped_genus=="Strongylocentrotus" | grouped_genus=="Balanophyllia") #select only the coocurring genus you want to look at (BASED ON INPUT)

neighborly <- nrow(reef_neighbor)

#Create basic table
reef_table <- as.data.frame(cbind(present, neighborly)) %>% 
  mutate(percent = neighborly/present)

#Generate nice table using `gt`
reef_table %>% 
  gt() %>% 
  fmt_percent(columns=vars(percent), decimal=2) %>% 
  cols_label(present="Focal species present",
             neighborly="Species present together",
             percent="Percent co-occurrence")
```
