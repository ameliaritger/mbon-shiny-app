...
#### Subset for a phylum MAYBE ADD DISTINCT()??
...
#### Subset for a location MAYBE ADD DISTINCT()??
```{r, message=FALSE, warning=FALSE}
...
reef_present <- reef_tidy %>%
  st_drop_geometry() %>% #remove sticky geometry because we don't need it
  filter(binary > "0") %>% 
  mutate(FOCAL="Corynactis") %>% #pick a focal genus (BASED ON INPUT)
  ...
  distinct(filename) #remove duplicate plots

present <- nrow(reef_present)

#Find number of times neighbor genera make an appearance
reef_neighbor_present <- reef_tidy %>%
  st_drop_geometry() %>% #remove sticky geometry because we don't need it
  filter(binary > "0") %>% 
  mutate(FOCAL1="Strongylocentrotus") %>% #pick a focal genus (BASED ON INPUT)
  mutate(FOCAL2="Balanophyllia") %>% #pick a focal genus (BASED ON INPUT)
  mutate(FOCAL3="Balanus") %>% #pick a focal genus (BASED ON INPUT)
  mutate(to_match = ifelse(grouped_genus==FOCAL1|grouped_genus==FOCAL2|grouped_genus==FOCAL3, filename, "FALSE")) %>% #create a column that we can subset all rows in a plot based on the presence of focal genera in the plot at least once
...
absent <- nrow(reef_neighbor_present)
...
reef_neighbor <- reef_tidy %>%
  ...
  mutate(to_match = ifelse(grouped_genus==FOCAL, filename, "FALSE")) %>% #create a column that we can subset all rows in a plot based on the presence of focal genus in the plot at least once
  filter(filename %in% to_match) %>% #if focal genus is present, keep all observations of that plot ("filename")
  filter(grouped_genus=="Strongylocentrotus" | grouped_genus=="Balanophyllia" | grouped_genus=="Balanus") %>% #select only the coocurring genera you want to look at (BASED ON INPUT)
  distinct(filename)

neighborly <- nrow(reef_neighbor)
...
reef_table %>% 
  gt() %>% 
  fmt_percent(columns=vars(percent_occur, percent_absent), decimal=2) %>% 
  cols_label(present="Focal genus present",
             ...
             percent_absent = "Percent times neighbor genera occur without focal genus")
```
