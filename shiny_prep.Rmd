...
reef_phylum <- reef_tidy %>%
  ...
  mutate(focal_phylum="cnidaria") %>% #pick a focal phylum (BASED ON INPUT)
  ...
  filter(location=="Rodes") %>% 
  filter(phylum %in% c("arthropoda", "chordata", "echinodermata")) %>% 
  distinct(filename, phylum, .keep_all=TRUE) #remove duplicate values within the same plot
```
