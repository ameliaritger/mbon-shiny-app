...
reef_tidy <- reef_tidy %>%
  ...
  separate(species, into="genus", sep="_", remove=FALSE) %>% #Add column for phylum name
...
reef_new <- reef_tidy %>% 
  mutate(newgenus = ifelse(str_detect(species, pattern = " worm"), "Other worms", ifelse(str_detect(species, pattern = "tubeworm"), "Tubeworms", species))) #convert all species with "worm"/"tubeworm" in the name to other worms/tubeworms
                           
                           

  ifelse((str_detect(species), pattern = "worm")="worm", TRUE, FALSE)
