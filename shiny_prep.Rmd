...
reef_vegan<- reef_tidy %>% #named so because of the vegan package!
  ...
  summarize(mean_count = mean(value)) %>%  #get the mean count
  select(location, grouped_species, mean_count, geometry)
...
reef_vegan <- reef_vegan %>% 
  pivot_wider(names_from=grouped_species, values_from=mean_count) %>% #get unique values of location (and ignore the species...)
