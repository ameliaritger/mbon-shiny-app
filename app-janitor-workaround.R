...
reef_tidy <- reef %>%
  ...
  pivot_longer(`Annelida-Cirriformia luxuriosa`:`Substrate-amphipod tube complex`) %>% 
  #clean_names() %>% 
#"annelida_cirriformia_luxuriosa":"substrate_amphipod_tube_complex") %>%  #make long form
  separate(name, into="phylum", sep="-", remove=FALSE) %>% #Add column for phylum name
  #mutate(vectorized_name=str_split(name, pattern="_")) %>% #In case this is useful...
  filter(!phylum=="No Data") %>% #remove values related to data collection issue
...
reef_tidy$species <- gsub(".*-","",reef_tidy$name, perl=TRUE) #Add column for species name
reef_tidy$genus <- gsub(" .*","",reef_tidy$species, perl=TRUE) #Add column for genus name
...
reef_vegan <- reef_tidy %>% #named so because of the vegan package!
  group_by(location,species) %>% #group by location, then lat/long
  summarize(mean_count = mean(value)) %>%  #get the mean count
  select(location, species, mean_count) %>% 
  ungroup()
                          ...
                          sidebarLayout(
                            sidebarPanel("",
                                         ...
                                         selectizeInput("searchaphylum", 
                                                        ...
                                                        choices = sort(c(unique(reef_tidy$genus), unique(reef_tidy$species))), 
                                                        multiple = FALSE,
                          ...
                          sidebarLayout(
                            sidebarPanel("",
                                         selectizeInput(inputId="mapitgenus",
                                                        ...
                                                        choices = sort(c(unique(reef_tidy$species), unique(reef_tidy$phylum))),
                                                        multiple = FALSE,
                                                        selected = 'Annelida'),
                                         br(),
...
reef_summary_genus_abund <- reactive({
  reef_tidy %>%
    filter(binary > "0") %>% #filter out species not present
    st_as_sf(coords=c("longitude", "latitude"), crs=4326) %>%  #create sticky geometry for lat/long
    filter((species==input$mapitgenus)|(phylum==input$mapitgenus)) %>% 
